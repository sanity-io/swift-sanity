...
class SanityClient {
    ...
    struct Query<T: Decodable> {
        ...
        struct ListenPublisher<T: Decodable>: Publisher {
            ...
            typealias Output = ListenResponse<T>
        ...
        class ListenSubscription<Target: Subscriber, T: Decodable>: Subscription where Target.Input == ListenResponse<T> {
            ...
            init(eventSource: EventSource, reconnect: Bool) {
    ...
    init(projectId: String, dataset: String, version: Config.APIVersion = .v20210325, apiKey: String? = nil) {
